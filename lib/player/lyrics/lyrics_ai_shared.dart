...
final class LyricsAiPromptBuilder {
  ...
  static String buildConvertToKaraokePrompt({
    ...
    return '这是这首歌的歌词和音频，请将歌词转换为卡拉OK逐字时间轴格式（word-by-word LRC / Enhanced LRC）。\n'
        ...
        '4. 【重要时间戳格式】：时间戳格式严格为 [mm:ss.xx]，其中第一位 mm 是【分（00-99）】，第二位 ss 是【秒（00-59）】，xx 是百分秒/毫秒。必须特别注意：第一位是分不是秒！绝对不能把第一位当成秒！例如歌曲在第 4.15 秒发音，时间戳必须是 [00:04.15]，绝对不能写成 [04:15.08]（[04:15.08] 表示 4分15秒，是严重的时间轴错乱）！未达到 1 分钟时，第一位分钟位必须是 00（例如 15 秒是 [00:15.00]）！\n'
        '5. 【空格规范】：中文、日文、韩文等非英文歌词，字与字之间、字与时间戳之间严禁添加任何空格（例如必须输出 [00:14.39]繋[00:14.65]い[00:15.00]だ，严禁输出 [00:14.39] 繋[00:14.65] い 或 [00:14.39]繋 [00:14.65]い 等多余空格格式）！对于歌词中的英文单词，单词与单词之间必须保留正常空格。\n'
        '6. 保持歌词文本内容原样不变，不要翻译，不要增加或删减任何歌词行，不要输出任何解释说明或Markdown代码块以外的内容。\n'
        '待转换歌词如下：\n'
        '$targetLyrics';
  }
